...
parser.add_argument('--model-path', type=str, default="best.pt", help='Path to the model')
...
def main(img_path):
    ...
    for target_layer in target_layers:
     ...
     for i, mask in enumerate(masks):
         ...
         res_img = put_text_box(bbox, "", res_img)
    ...
    for item in os.listdir(folder_path):
        ...
        for target_layer in target_layers:
         ...
         for i,image in enumerate(images):
             final_image = image
             img_name = split_extension(os.path.split(img_path)[-1], suffix='_res_'+target_layer+str(i))
             output_path = f'{args.output_dir}/{img_name}'
             os.makedirs(args.output_dir, exist_ok=True)
             print(f'[INFO] Saving the final image at {output_path}')
             cv2.imwrite(output_path, final_image)
