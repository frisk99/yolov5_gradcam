...
with gr.Blocks() as demo:
    ...
    with gr.Tab(label="Editing Real Image"):
        ...
        img_name = gr.State(value=None) # store the name of the image
        with gr.Row():
            with gr.Column():
                gr.Markdown("""<p style="text-align: center; font-size: 20px">Draw Mask</p>""")
                canvas = gr.Image(type="filepath", tool="sketch", label="Draw Mask", #filepath
    ...
    #                 label="Diffusion Model Path",
    #                 choices=[
    #                     r"G:\huggingface\stable-diffusion-v1-5"
    #                 ] + local_models_choice
    #             )
    #             vae_path_gen = gr.Dropdown(value="defaulrt",
    #                 label="VAE choice",
    #                 choices=["default",
    #                 r"G:\huggingface\sd-vae-ft-mse"] + local_models_choice
    #             )
    ...
    canvas.edit(
        store_img,
        [canvas],
        [original_image, selected_points, input_image, mask,img_name]
    )
    input_image.select(
        get_points,
