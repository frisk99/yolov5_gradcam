...
def store_img(img, length=512):
    ...
    img_name = img["image"].split('\\')[-2]
    print(img_name)
    img_o = Image.open(img["image"])
    #img_o = img_o.resize((LENGTH,LENGTH))
    img_o = np.array(img_o, dtype=np.uint8)
    img_om = Image.open(img["mask"])
    img_om = np.array(img_om, dtype=np.uint8)
    image, mask = img_o, np.float32(img_om [:, :, 0]) / 255.
    ...
    return image, [], gr.Image.update(value=masked_img, interactive=True), mask, img_name
...
def run_drag(source_image,
    ...
    # set lora
    if os.path.exists(os.path.join(lora_path,img_name+'.safetensors')) == False:
        print("applying default parameters")
        model.unet.set_default_attn_processor()
    else:
        print('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        print("applying lora: " + os.path.join(lora_path,img_name+'.safetensors'))
        model.unet.load_attn_procs(os.path.join(lora_path,img_name+'.safetensors'))
